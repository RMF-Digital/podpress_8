...
Info for WordPress:
==============================================================================
Plugin Name: podPress_8
Version: 8.8.20
Plugin URI: 
Description: The podPress plugin gives you everything you need in one easy plugin to use WordPress for Podcasting. Set it up in <a href="admin.php?page=podpress_feed">'podPress'->Feed/iTunes Settings</a>. If this plugin works for you, send us a comment.
Author: RMF DIGITAL
Author URI: http://rmfdigital.com.br/
Min WP Version: 2.3
...
function podPress_init() {
	...
	if ( TRUE == version_compare($wp_version, '2.8', '>=') ) {
		...
		add_action('widgets_init', function() { return register_widget("podpress_feedbuttons"); });
		add_action('widgets_init', function() { return register_widget("podpress_xspfplayer"); });
	} else {
...
function podPress_class_init() {
	...
	if ( TRUE === is_admin() ) {
		...
		if ( TRUE == version_compare($wp_version, '2.7', '>=') ) {
			...
			add_action('admin_print_scripts-podpress_page_podpress_stats', 'podpress_print_admin_statistics_js');
			...
			add_action('admin_print_styles-podpress_page_podpress_stats', 'podpress_print_admin_statistics_css');
			...
			if ( podPress_isset_upgrade_status('podpress_update_stats_table') OR podPress_isset_upgrade_status('podpress_update_statcounts_table') ) {
				...
				add_action('admin_print_scripts-podpress_page_podpress_general', 'podpress_print_admin_gs_js');
			}
		} else {
...
function podPress_add_menu_page() {
	...
	if (function_exists('add_menu_page')) {
		...
		add_menu_page('podPress', __('podPress', 'podpress'), $permission_needed, $starting_point, 'podPress_render_page_'.$starting_point, plugins_url('images/podpress_icon_r2_v2_16.png', __FILE__) );
	}
	if (function_exists('add_submenu_page')) {
		...
		if($podPress->settings['enableStats'] == true) {
			add_submenu_page($starting_point, __('podPress - Statistics', 'podpress'), __('Statistics', 'podpress'), $permission_needed, 'podpress_stats', 'podPress_render_page_podpress_stats');
		}
		add_submenu_page($starting_point, __('podPress - Feed/iTunes Settings', 'podpress'), __('Feed/iTunes Settings', 'podpress'), $permission_needed, 'podpress_feed', 'podPress_render_page_podpress_feed');
		add_submenu_page($starting_point, __('podPress - General Settings', 'podpress'), __('General Settings', 'podpress'), $permission_needed, 'podpress_general', 'podPress_render_page_podpress_general');

		if($podPress->settings['contentPlayer'] != 'disabled') {
			add_submenu_page($starting_point, __('podPress - Player Settings', 'podpress'), __('Player Settings', 'podpress'), $permission_needed, 'podpress_players', 'podPress_render_page_podpress_players');
		}

		if(!empty($podPress->settings['enablePodangoIntegration']) && $podPress->settings['enablePodangoIntegration'] == true) {
			add_submenu_page($starting_point, __('podPress - Podango Settings', 'podpress'), __('Podango Settings', 'podpress'), $permission_needed, 'podpress_podango', 'podPress_render_page_podpress_podango');
		}
	}
}

function podPress_render_page_podpress_feed() {
	global $podPress;
	include(PODPRESS_DIR . '/podpress_feed.php');
}
function podPress_render_page_podpress_general() {
	global $podPress;
	include(PODPRESS_DIR . '/podpress_general.php');
}
function podPress_render_page_podpress_players() {
	global $podPress;
	include(PODPRESS_DIR . '/podpress_players.php');
}
function podPress_render_page_podpress_stats() {
	global $podPress;
	include(PODPRESS_DIR . '/podpress_stats.php');
}
function podPress_render_page_podpress_podango() {
	global $podPress;
	include(PODPRESS_DIR . '/podpress_podango.php');
}
