...
class podPressAdmin_class extends podPress_class {
	// Declared properties to avoid PHP 8.2 dynamic property deprecation
	public $path           = '';
	public $wpdb           = null;
	...
	function __construct() {
		...
		$this->path = site_url().'/wp-admin/admin.php?page=podpress_stats';
	...
	function settings_stats_edit() {
		...
		$baseurl = site_url() . '/wp-admin/admin.php?page=podpress_stats&display=';
		echo '<div class="wrap">'."\n";
	...
	function graphByDate() {
		...
		if ($this->checkWritableTempFileDir() && $this->checkFontFile()) {
			...
			if (($value_max > 0) && ($cnt_days >= $min_downloaddays)) {
				...
				imagettftext($image, 8, 0, $sidemargin, 42, $colors['text'], $this->fontface, sprintf(__('last %1$s weeks', 'podpress'), $weeks).' &#187; '.date('d.m.Y', $first_day).' - '.date('d.m.Y', $last_day));
				...
				imagettftext($image, 6, 90, ($image_width - 15), ($image_height - 10), $colors['copytext'], $this->fontface, podpress_safe_strftime($this->local_settings['creation_date'], time()).' ('.PODPRESS_VERSION.')');
				...
				foreach ($stats AS $day => $idata) {
					...
					if ($weekday == 1) {
						...
						imagettftext($image, 8, 0, ($pos_x + 3), ($baseline + 13), $colors['text'], $this->fontface, podpress_safe_strftime($this->local_settings['short_date'], strtotime($day)));
	...
	function graphByPost() {
		...
		if ($this->checkWritableTempFileDir() && $this->checkFontFile()) {
			...
			if (intval($value_max) > 0) {
				...
				/* create the legend */
				imagettftext($image, 14, 0, $sidemargin, 25, $colors['text'], $this->fontface, get_option('blogname'));
				imagettftext($image, 8, 0, $sidemargin, 42, $colors['text'], $this->fontface, __('Posts', 'podpress').' &#187; '.date('d.m.Y', $first_post).' - '.date('d.m.Y', $last_post));
				...
				foreach ($data AS $idata) {
					...
					imagettftext($image, 8, 0, ($pos_x - $col_width - 3), ($baseline + 14), $colors['text'], $this->fontface, podpress_safe_strftime($this->local_settings['short_date'], $idata->post_date));
