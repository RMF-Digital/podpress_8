...
*/
	class podPress_class {
		// Properties (var converted to public for PHP 8.2 compatibility)
		public $settings            = array();
		public $podcasttag_regexp   = "/\[podcast:([^]]+)]/";
		public $podcasttag          = '[display_podcast]';
		public $podtrac_url         = 'http://www.podtrac.com/pts/redirect.mp3/';
		public $blubrry_url         = 'http://media.blubrry.com/';
		public $requiredadminrights = 'manage_categories';
		public $realm               = 'Premium Subscribers Content';
		public $justposted          = false;
		public $uploadurl           = '';
		public $uploadpath          = '';
		public $tempfilesystempath  = '';
		public $tempfileurlpath     = '';
		public $tempcontentaddedto  = array();
		public $podangoapi          = null;
		public $isexcerpt           = false;
		...
		*/
		...
		function checkSettings() {
			...
			// Default for Podango integration (may not exist in older installs)
			if(!isset($this->settings['enablePodangoIntegration'])) {
				$this->settings['enablePodangoIntegration'] = false;
			}
			if(!isset($this->settings['podangoUserKey'])) {
				$this->settings['podangoUserKey'] = '';
			}
			if(!isset($this->settings['podangoPassKey'])) {
				$this->settings['podangoPassKey'] = '';
			}
		...
		*/
		...
		function addPostData($input, $forEdit = false) {
			...
			if(is_array($input->podPressMedia)) {
				...
				foreach (array_keys($input->podPressMedia) as $key) {
		...
		*/
		...
		function insert_content($content = '', $is_the_excerpt = FALSE) {
			...
			foreach (array_keys($post->podPressMedia) as $key) {
	...
	function millisecondstostring($duration, $format = 'h:m:s', $dimensionchr = ':', $decimals = 3, $dec_point = '.') {
		...
		Switch ($format) {
			...
			case 'h:m:s' :
				...
				$dur['m'] = intval((int)($duration / 60000) % 60);
				$dur['s'] = intval((int)($duration / 1000) % 60);
			...
			case 'h:m:s:ms' :
				...
				$dur['m'] = intval((int)($duration / 60000) % 60);
				$dur['s'] = intval((int)($duration / 1000) % 60);
				$dur['ms'] = intval((int)$duration % 1000);
				return $this->millisecondstostring_print($dur, $duration, $this->podpress_build_dimchr($dur, $dimensionchr));
			break;
			case 'm:s:ms' :
				$dur['m'] = intval($duration / 60000);
