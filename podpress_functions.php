...
/**
 * PHP 8.1+ compatibility: strftime() was deprecated in PHP 8.1 and removed in PHP 8.2+
 * This function converts strftime format strings to date() format strings.
 */
if (!function_exists('podpress_safe_strftime')) {
	function podpress_safe_strftime($format, $timestamp = null) {
		if ($timestamp === null) {
			$timestamp = time();
		}
		// Map strftime format specifiers to date() format specifiers
		$mapping = array(
			'%A' => 'l',    // Full weekday name
			'%a' => 'D',    // Abbreviated weekday name
			'%B' => 'F',    // Full month name
			'%b' => 'M',    // Abbreviated month name
			'%C' => '',     // Century (not supported, ignored)
			'%c' => 'D M j H:i:s Y', // Preferred date and time
			'%D' => 'm/d/y',// Same as %m/%d/%y
			'%d' => 'd',    // Day of month (zero-padded)
			'%e' => 'j',    // Day of month (space-padded, use j)
			'%F' => 'Y-m-d',// ISO 8601 date
			'%G' => 'o',    // ISO 8601 week-based year
			'%g' => '',     // 2-digit ISO week year (not directly supported)
			'%H' => 'H',    // Hour 24h (zero-padded)
			'%h' => 'M',    // Same as %b
			'%I' => 'h',    // Hour 12h (zero-padded)
			'%j' => 'z',    // Day of the year (note: strftime is 1-based, date() is 0-based)
			'%k' => 'G',    // Hour 24h (space-padded, use G)
			'%l' => 'g',    // Hour 12h (space-padded, use g)
			'%M' => 'i',    // Minutes
			...
			'%P' => 'a',    // am/pm
			'%R' => 'H:i',  // Time as HH:MM
			'%r' => 'h:i:s A', // Time as hh:mm:ss AM/PM
			'%S' => 's',    // Seconds
			'%s' => 'U',    // Unix timestamp
			'%T' => 'H:i:s',// Time as HH:MM:SS
			'%t' => "\t",   // Tab
			'%U' => 'W',    // Week number (Sunday first, use W as approximation)
		...
		$date_format = str_replace(array_keys($mapping), array_values($mapping), $format);
	...
	function podPress_url($noDomain = false) {
		if($noDomain) {
			...
			//~ if(substr($result, -1, 1) != '/') {
				//~ $result .= '/';
			//~ }
			//~ return $result.'wp-content/plugins/podpress_8/';
			return PODPRESS_URL.'/';
		}
	}
	...
	if ( TRUE == version_compare($wp_version, '2.8', '>=') ) {
		class podpress_feedbuttons extends WP_Widget {
			...
			function __construct() {
				...
				parent::__construct('podpress_feedbuttons', __('podPress - Feed Buttons','podpress'), $widget_ops, $control_ops);
			}
	...
	*/
	if ( TRUE == version_compare($wp_version, '2.8', '>=') ) {
		class podpress_xspfplayer extends WP_Widget {
			...
			function __construct() {
				...
				parent::__construct('podpress_xspfplayer', __('podPress - XSPF Player','podpress'), $widget_ops, $control_ops);
			}
	...
	function podPress_remote_version_check() {
		...
		if(!is_array($latestVersionCache)) {
			$latestVersionCache = array('cached' => 0, 'version' => $current);
		}
